...
module.exports = class Workspace {
    ...
    getRelativePath(path) {
        return Path.resolve(path).replace(this.path, '');
    }
    ...
    async inNotebook(path) {
        console.log('check in notebook', path);
        while(this.inWorkspace(path)) {
            ...
            if(stats.isDirectory()) {
                ...
                if(files.includes('book.json')) {
                    return await this.parse(Path.join(path, 'book.json'), 0);
                }
            }
    ...
    async parse(path, depth = 1) {
        ...
        console.log(path, inNodeModules, inNodeModulesRoot);
        ...
        if (stats.isDirectory()) {
            ...
            } else if (content.includes('book.json')) {
                const bookData = await fsp.readFile(Path.join(path, 'book.json'));
    ...
    resolve(path) {
        console.log(this.path, path, Path.resolve(this.path, path));
